...
using Fcl.Net.Maui.Platforms.Android;
...
namespace Fcl.Net.Maui
{
    ...
    {
        TaskCompletionSource<FclAuthResponse> tcsResponse = null;
        ...
        public bool OnResumeCallback(Intent intent)
        {
            ...
            try
            {
                ...
                //TODO - response??
                tcsResponse?.TrySetResult(new FclAuthResponse());
                return true;
            }
            catch (Exception ex)
            {
        ...
        public async Task<FclAuthResponse> OpenWebBrowser(Uri url, Uri callbackUrl)
        {            
            ...
            tcsResponse = new TaskCompletionSource<FclAuthResponse>();
        ...
        static async Task<bool> StartCustomTabsActivity(Uri url)
        {
            ...
            var success = false;
            var parentActivity = ActivityStateManager.Default.GetCurrentActivity();
            ...
            try
            {
                if (await BindServiceAsync(customTabsActivityManager))
                {
                    ...
                    {
                        WebAuthIntermediateActivity.StartActivity(parentActivity, customTabsIntent.Intent);
                        success = true;
                    }
                }
            }
